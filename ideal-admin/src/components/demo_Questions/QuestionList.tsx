...
const OptionContent = ({
  ...
  if (option.type === 'LATEX') {
    return <MathPreview text="" latex={option.answer} />;
  }
  return <span className="text-gray-800">{option.answer}</span>;
}; 

const QuestionList = () => {
  const navigate = useNavigate();
  const router = useRouter();
  const search: any = useSearch({ from: '/demo/addquestion/' });
  const {
    demoCourseId,
    page = 1,
    limit = 10,
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await _axios.get(
        `/course-demo/all-questions?${params.toString()}`,
      );
  ...
  const deleteMutation = useMutation({
    mutationFn: (questionId: string) =>
      _axios.delete(`/course-demo/deletequestion?questionId=${questionId}`),
    onSuccess: () => {
      refetch();
