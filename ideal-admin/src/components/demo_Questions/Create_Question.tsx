...
const CreateDemoMathsQuiz = ({
  ...
  const { data: existingQuestion, isLoading: isLoadingQuestion } = useQuery({
    queryKey: ['demo-question-Id', questionId],
    queryFn: async () => {
      const response = await _axios.get(`/course-demo/question/${questionId}`);
  ...
  useEffect(() => {
    if (existingQuestion && isEditMode) {
      setFormData({
        demoCourseId: existingQuestion.demoCourseId,
  ...
  const buildPayload = (
    ...
    if (needsFormData) {
      ...
      fd.append('demoCourseId', data.demoCourseId);
  ...
  const mutation = useMutation({
    mutationFn: async (data: CreateQuestionSchema) => {
      ...
      if (isEditMode) {
        return _axios.patch(
          `/course-demo/update-question/${questionId}`,
          payload,
          config,
        );
      }
      return _axios.post('/course-demo/add-question', payload, config);
    },
    onSuccess: (data) => {
...
export default CreateDemoMathsQuiz;
