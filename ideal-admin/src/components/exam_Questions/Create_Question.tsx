...
const CreateMathsQuiz = ({
  ...
  const { data: existingQuestion, isLoading: isLoadingQuestion } = useQuery({
    queryKey: ['exam-question-Id', questionId],
    queryFn: async () => {
      const response = await _axios.get(`/exam/question/${questionId}`);
  ...
  const mutation = useMutation({
    mutationFn: async (data: CreateQuestionSchema) => {
      ...
      if (isEditMode) {
        return _axios.patch(
          `/exam/update-question/${questionId}`,
          payload,
          config,
        );
      }
      return _axios.post('/exam/add-question', payload, config);
    },
    onSuccess: (data) => {
