...
const TaskReviewPage: React.FC = () => {
  const { submissionId } = useParams({
    from: '/tasks/submissions/$submissionId',
  });
  ...
  const handleReject = () => {
    if (!feedback.trim()) {
      toast('Feedback is required when rejecting');
      return;
    }
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-7xl">
      <Button
        ...
        variant="outline"
        className="rounded-xsm my-2 hover:text-foreground text-foreground  cursor-pointer hover:bg-accent/50">
        <ArrowBigLeft className="mr-2 h-4 w-4" /> Back to Submissions
      </Button>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        // Handle both url and urls format
                        const pdfUrl =
                          latestSubmission.submissionData?.url ||
                          latestSubmission.submissionData?.urls?.[0];

                        if (pdfUrl) {
                          window.open(pdfUrl, '_blank', 'noopener,noreferrer');
                        } else {
                          console.error('No PDF URL found:', latestSubmission);
                          ...
                          submission.rejectedItems.length > 0 && (
                            <div>
                              ...
                              <div className="space-y-2">
                                {submission.rejectedItems.map((item, idx) => (
                                  <div
                                    key={idx}
                                    ...
                                    <div className="flex items-start gap-2">
                                      ...
                                      <div className="flex-1">
                                        <p className="text-sm font-medium text-red-900">
                                          {item.type === 'image' &&
                                            `Image ${typeof item.index === 'number' ? item.index + 1 : ''}`}
                                          {item.type === 'pdf' &&
                                            'PDF Document'}
                                          {item.type === 'link' &&
                                            'Google Drive Link'}
