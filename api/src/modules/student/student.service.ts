...
export const adminUpdateStudentDetails = async (
    ctx: Context<{ body: AdminStudentUpdateInput }>
) => {
    const { body, set, } = ctx;
    const { studentProfile, ...updateFields } = body;
    ...
    try {
        // Check if student exists
        const existingStudent = await studentCollection.findOne({
            _id: new ObjectId(updateFields.id),
        });
        console.log(updateFields.id, "sss")
        ...
        if (updateFields.email) {
            duplicateCheck.$or.push({
                email: updateFields.email.toLowerCase(),
                _id: { $ne: new ObjectId(updateFields.id) },
            });
        }
    ...
    try {
        ...
        return {
            student: {
                ...
                boardId: student.nameOfTheBoard,
                boardName: boardDetails?.boardName || null,
                grade: gradeDetails?.grade || null,
                enrolledCourses
            },
...
export const toggeleStudentStatusById = async (ctx: Context<{ params: { studentId: string } }>) => {
    ...
    try {
        ...
        return {
            message: `Student has been ${newStatus ? 'activated' : 'deactivated'} successfully`,
        };
    } catch (error) {
        console.log("Student Deactivated Error", error);
        set.status = 500;
